...
const defaultColors = ['#60a5fa', '#60a5fa', '#a78bfa'];
...
const vertex = `attribute vec3 position; attribute vec4 random; attribute vec3 color;
...
export default function Particles({
  ...
  disableRotation = false, pixelRatio = 1,
}) {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ dpr: Math.min(pixelRatio, window.devicePixelRatio || 1), depth: false, alpha: true });
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uBaseSize: { value: particleBaseSize * Math.min(pixelRatio, window.devicePixelRatio || 1) },
    ...
    let raf, lastTime = performance.now(), elapsed = 0;
    const update = t => {
      ...
      else { particles.position.x = 0; particles.position.y = 0; }
